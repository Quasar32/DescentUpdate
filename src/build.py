...
def comp(source_path):
    comp_path = 'gcc -c ' + source_path + ' -Wall -g -O3'
    print(comp_path)
    os.system(comp_path) 

def build():
    for source_path, header_paths in flat_dict.items():
        object_path = source_path.replace('.c', '.o')
        source_modify = os.path.getmtime(source_path) 
        try:
            object_modify = os.path.getmtime(object_path)
            if source_modify > object_modify: 
                comp(source_path)
                break
            for header_path in header_paths: 
                header_modify = os.path.getmtime(header_path)
                if header_modify > object_modify:
                    comp(source_path)
                    break
        except:
            comp(source_path)
            break
    os.system('gcc *.o -o ../build/descent -mwindows')
...
def create_makefile():
    with open('makefile', 'w') as f:
        ...
        f.write('\tgcc $(OBJFILES) -o ../build/descent $(LINKFLAGS)\n')
